...
class NestedCV:
    ...
    def fit(self, data: FeatureSet):
        with utils.custom_tqdm(total=self.n_repeats) as pbar:
            ...
            for repeat in range(self.n_repeats):
                ...
                ) for k_outer, train_outer, valid_outer in cv_iterator(outer_cv, data, _process_kwargs, _rank_kwargs)]
    ...
    def _perform_inner_cv_bayes_opt(outer_fold: Fold, model_setup: ModelSetup, rank_map: dict, fit_params: dict,
                                    save_path: Path, inner_cv_seed: int, plot_search_history: bool = True):
        ...
        results = bayesian_opt.fit(outer_fold.x, outer_fold.y, **fit_params)
...
if __name__ == '__main__':
    ...
    nested_cv = NestedCV(config, save_path)
    nested_cv.eval()
